...
class CheckoutSolution {
    ...
    getFreeItems(itemCounts) {
        ...
        for (const [item, count] of Object.entries(itemCounts)) {
            const crossItemOffer = this.crossItemOffers[item];

            if (crossItemOffer) {
                const offerQuantity = parseInt(Object.keys(crossItemOffer)[0]);
                const freeItemDetails = crossItemOffer[offerQuantity];
